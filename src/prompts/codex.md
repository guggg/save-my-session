...
You have save-my-session installed. It converts Codex sessions into Claude Code / Gemini CLI native format so the next agent can pick up right where you left off — no re-briefing needed.

### When to hand off

When the user says they want to switch agents (e.g. "transfer to Claude", "switch to Gemini"), follow the handoff flow below.
...
1. Open the target CLI (`claude` or `gemini`)
