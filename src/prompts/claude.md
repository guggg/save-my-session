...
- 注意你的 API 使用量。當你感覺到回應速度明顯變慢、收到 rate limit 警告、或這個 session 已經進行超過 30 輪對話，主動提醒使用者：
  「⚠️ 額度可能快到上限了，要交接到其他 Agent 嗎？（Gemini CLI / Codex）」
- 如果使用者同意交接，執行以下步驟。
...
2. 執行指令將 briefing 存檔：
```bash
cat << 'BRIEFING_EOF' > .handoff-briefing.md
（上面生成的 briefing 內容）
BRIEFING_EOF
```
...
3. 告知使用者 briefing 已儲存，並提供啟動目標 Agent 的指令：
   ...
   - 轉到 Codex：`codex "請先讀取 .handoff-briefing.md 的內容，這是前一個 Agent 的交接文件，讀完後告訴我你理解了什麼，然後繼續工作。"`
